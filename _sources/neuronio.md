...
Para $r_1=10$, $r_2=1$, $r_3=6$, $\mu=10^{-4}$ e $M=2$, a saída do LMS no modo estocástico ($N_t=1000$, $N_b=1$ e $N_e=1$) está mostrada na Figura~\ref{fig:SaidaLMS}. Nesta aplicação, considerou-se como sinal desejado $d=+1$ para dados  pertencentes à Região A e $d=-1$ para os pertencentes à Região B. O LMS consegue classificar os dados com uma taxa de erros de aproximadamente 2,5\% com sua reta de separação entre as regiões. Apesar disso, a saída do algoritmo fica espalhada no intervalo $[-1,5\;\; 1,5]$, não havendo uma clara separação em torno do zero.
...
name: fig_SaidaLMS
---
Saída do algoritmo LMS ($\mu=10^{-4}$ e $M=2$) durante o treinamento no modo estocástico ($N_t=1000$, $N_b=1$ e $N_e=1$) utilizado  no problema de classificação das meias-luas ($r_1=10$, $r_2=1$ e $r_3=6$).
```
